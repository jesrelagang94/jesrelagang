...
export const customCursor = () => {
  var myCursor = document.querySelectorAll(".mouse-cursor"),
    ...
    ja_topbar = document.querySelector(".ja_topbar "),
      ...
      (window.onmousemove = function (s) {
        ...
        document.body.addEventListener(
          ...
          function () {
            ...
            ja_topbar && mouseEvent(ja_topbar);
...
export const scroll_ = () => {
  const sections = document.querySelectorAll(".ja_section");
...
export const stickyNav = () => {
  ...
  const stickys = document.querySelectorAll(".ja_header");
...
export const portfolioHover = () => {
  const ja_portfolio_animation_wrap = document.querySelectorAll(
      ".ja_portfolio_animation_wrap"
    ),
    ja_portfolio_titles = document.querySelector(
      ".ja_portfolio_titles"
    );
  ja_portfolio_animation_wrap.forEach((element) => {
    element.addEventListener("mousemove", () => {
      let title = element.getAttribute("data-title"),
        category = element.getAttribute("data-category");
      if (title) {
        ja_portfolio_titles.classList.add("visible");
        ja_portfolio_titles.innerHTML =
          title + '<span class="work__cat">' + category + "</span>";
      }
      document.addEventListener("mousemove", (e) => {
        ja_portfolio_titles.style.left = `${e.clientX - 10}px`;
        ja_portfolio_titles.style.top = `${e.clientY + 25}px`;
      });
    });
    element.addEventListener("mouseleave", () => {
      ja_portfolio_titles.classList.remove("visible");
    });
  });
};
